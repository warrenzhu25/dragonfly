...
class OAHPair {
 public:
  ...
  // Compares this pair's key against an encoded query key (see ascii::EncodedStr). Hot path: reads
  // the header inline rather than via StoredKey() to keep it register-friendly (see key::Matches).
  bool KeyMatches(const ascii::EncodedStr& k) const {
    const Header header = ParseHeader();
    return oah::key::Matches(header.key, header.key_content, k);
  }
