...
async def test_tls_full_duplex_large_pipeline(
    df_factory, with_ca_tls_server_args, with_tls_ca_cert_args, with_tls_client_args
):
    ...
    # The server verifies client certs (tls_ca_cert_file is set), so present the client cert
    # chain. We don't need to verify the server cert here (CN won't match 127.0.0.1).
    ssl_ctx = ssl.create_default_context(cafile=with_tls_ca_cert_args["ca_cert"])
    ...
    num_cmds = 300  # ~38 MiB streamed in each direction.
